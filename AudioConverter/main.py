# Program that converts audio files from one folder, from one format into another
...
parser.add_argument('-if', '--input_format', help='Input format', default='mp3')
parser.add_argument('-of', '--output_format', help='Output format', default='ogg')
...
for file in files:
    ...
    if file.lower().endswith(input_format.lower()):
        ...
        # Load the file
        sound = AudioSegment.from_file(full_path, input_format)
        # Get the name of the file without the extension
        name = os.path.splitext(file)[0]
        # Get the full path of the output file
        output_file = os.path.join(output_folder, name + '.' + output_format)
        # Save the file
        sound.export(output_file, format=output_format, parameters=["-c:a","libopus","-ar","16k","-b:a","16k","-vbr","on"])